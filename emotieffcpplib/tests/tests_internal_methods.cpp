...
TEST_P(EmotiEffLibInternalTests, ImagePreprocessing) {
    ...
    float expTensor[3][5][5] = {{{-0.7137, -1.9467, 0.6221, 0.4337, 0.0741},
    ...
    // Compare each value in the cv::Mat with the expected tensor
    const float tolerance = 1e-4f; // Allowable difference due to floating-point precision

    // Iterate through channels and check values
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
            for (int c = 0; c < 3; ++c) {
                // if (backend == "onnx") {
                //     EXPECT_NEAR(res.at<float>(c, i, j), expTensor[c][i][j], tolerance)
                //         << "Mismatch at (" << i << ", " << j << ", " << c << ")";
                //     EXPECT_NEAR(res.at<cv::Vec3f>(c, i)[j], expTensor[c][i][j], tolerance)
                //         << "Mismatch at (" << i << ", " << j << ", " << c << ")";
                // } else {
                EXPECT_NEAR(res.at<cv::Vec3f>(i, j)[c], expTensor[c][i][j], tolerance)
                    << "Mismatch at (" << i << ", " << j << ", " << c << ")";
                //}
            }
        }
